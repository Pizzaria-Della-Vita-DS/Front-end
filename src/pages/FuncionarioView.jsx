...
export default function FuncionarioView() {
	...
	useEffect(() => {
		const carregarDadosDoPainel = async () => {
			try {
				const [resSabores, resBordas, resIngredientes, resPedidos] = await Promise.all([
		...
		<div className="min-h-screen bg-gray-50/50 font-sans text-gray-900">
			...
			<header className="sticky top-0 z-40 bg-red-700 text-white shadow-md">
				<div className="max-w-7xl mx-auto px-4 sm:px-6 flex justify-between items-center h-16">
					...
					<div className="flex items-center gap-4">
						<div onClick={() => setCurrentPage('perfil')} className="flex items-center gap-3 bg-black/10 hover:bg-black/20 px-3 py-1.5 rounded-full cursor-pointer transition-colors">
							<div className="w-8 h-8 bg-white text-red-700 rounded-full flex items-center justify-center font-bold text-sm">J</div>
							<div className="hidden lg:block text-sm">
								<div className="font-semibold leading-none">João Pereira</div>
								<div className="text-[11px] text-red-100 mt-0.5">Funcionário</div>
							</div>
						</div>
				...
				<nav className="md:hidden flex w-full border-t border-red-600/50 bg-red-700 overflow-x-auto">
					{['pedidos', 'sabores', 'bordas', 'estoque'].map(tab => (
						<button
							key={tab}
							...
							<div className="max-w-3xl mx-auto p-6 animate-fade-in">
								...
								<div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
									<div className="flex items-center gap-6 border-b border-gray-100 pb-8 mb-8">
										<div className="w-20 h-20 rounded-full bg-red-100 text-red-700 flex items-center justify-center text-3xl font-bold">J</div>
										<div>
											<h2 className="text-2xl font-bold text-gray-900">João Pereira</h2>
											<p className="text-gray-500 font-medium">Pizzaiolo</p>
										</div>
									</div>
									<div className="space-y-6">
										{[
											{ label: 'Nome', value: 'João Pereira', edit: true }, 
											{ label: 'E-mail (Login)', value: 'joao.pereira@pizzaria.com', edit: true },
											{ label: 'Senha', value: '••••••••', edit: true },
											{ label: 'CPF', value: '111.222.333-44' }, 
											{ label: 'Telefone', value: '(53) 99999-8888', edit: true },
											{ label: 'Data de Nascimento', value: '15/04/1985' },
											{ label: 'RG', value: '3456789012' },
											{ label: 'Gênero', value: 'Masculino' },
											{ label: 'Setor', value: 'Cozinha' }
										].map((info, idx) => (
